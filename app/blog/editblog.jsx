...
import { createBlog, editBlog } from '../../lib/appwrite';

const editblog = () => {
    const { user, blog} = useGlobalContext();

    const [form, setForm] = useState({ 
        title: blog.title,
        text: blog.text,
        currentThumbnails: blog.thumbnails,
        blogId: blog.$id,
        likes: blog.likes,
        saves: blog.saves,
        newThumbnails: [],
    ...
    const submit = async () => {
        if(!form.title || !form.text || !form.newThumbnails) {
            return Alert.alert('Please fill in all fields')
        }
        setisSubmitting(true);
        
        try {
            await editBlog({...form, userId: user.$id});
            Alert.alert('Success', 'Blog has been updated!')
  ...
  return (
    <SafeAreaView className="bg-tertiary h-full w-full px-4 pb-4">
        <View className="flex-row justify-between items-center border-b-2 border-primary pb-4 mt-2 mb-4 ">
            <Text className="font-lobster text-4xl text-secondary">Edit</Text>
        ...
        <ScrollView>
            <FormField title={"Title"} inputStyles={"text-primary-alt text-xl"} value={form.title} containerStyles={""} textStyles={"font-opensansItalic text-secondary"} handleChangeText={(e) => setForm({...form, title: e})}/>
            <FormField title={"Text"} inputStyles={"text-primary-alt text-xl"} value={form.text} containerStyles={"mt-5"} inputContainerStyles={"p-2 h-40 mt-5 items-start"} handleChangeText={(e) => setForm({...form, text: e})} multiline={true} textStyles={"font-opensansItalic text-secondary"}/>
            <View className="flex-row items-center mt-5 mb-2 border-b-2 border-primary-alt pb-3">
                ...
                <Text className="ml-2 text-secondary bg-primary px-2.5 py-0.5 rounded-lg font-opensansRegular text-lg">{form.currentThumbnails?.length + form.newThumbnails?.length}</Text>
            </View>
            ...
            {form.thumbnailView && form.thumbnailView.uri ? (
                <CustomButton title={!form.thumbnailView.uri.includes('mp4') ? "Add Image": "Add Video" } containerStyles={"bg-primary-alt mt-5 h-[45px] w-1/2"} textStyles={"text-lg text-secondary pt-1 font-lobster"} handlePress={() => setForm({...form, newThumbnails:[...form.newThumbnails, form.thumbnailView], thumbnailView: '' })}/>
            ):null}
            <View className="border-t-2 border-primary-alt mt-6 pt-5">
                <CustomButton title="Update" textStyles={"text-xl text-secondary pt-1 font-lobster"} containerStyles={"bg-primary"} isLoading={isSubmitting} handlePress={submit}/>
            </View>
        </ScrollView>
    </SafeAreaView>
  )
