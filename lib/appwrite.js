...
export const getCurrentUser = async() => {
    try {
        ...
        if(!currentAccount) throw new Error('user does not exists');
    ...
    try {
        ...
        for (let x = 0; x < blog.thumbnails.length; x++) {
            if(blog.thumbnails[x].uri.includes('mp4')) {
                const data = await uploadFile(blog.thumbnails[x], 'video')
                thumbnailUrls.push(data[0]);
                thumbnailIds.push(data[1])
            } else {
                const data = await uploadFile(blog.thumbnails[x], 'image')
                thumbnailUrls.push(data[0]);
                thumbnailIds.push(data[1])
            }
        }
        console.log(thumbnailUrls, blog)
